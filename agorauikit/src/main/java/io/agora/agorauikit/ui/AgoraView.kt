...
class AgoraView @JvmOverloads constructor(
    ...
    fun setRenderMode(@RenderMode renderMode: Int) {
        ...
        if (canvas.uid == 0) {
            AgoraRTC.instance()._sdk?.setLocalRenderMode(canvas.renderMode, canvas.mirrorMode)
        } else {
            AgoraRTC.instance()._sdk?.setRemoteRenderMode(canvas.uid, canvas.renderMode, canvas.mirrorMode)
        }
    }
    ...
    fun setChannelId(channelId: String) {
        ...
        if (canvas.uid == 0) {
            AgoraRTC.instance()._sdk?.setupLocalVideo(canvas)
        } else {
            AgoraRTC.instance()._sdk?.setupRemoteVideo(canvas)
        }
    }
    ...
    fun setUID(uid: Int) {
        mUID = uid
        if (uid == 0) {
            AgoraRTC.instance()._sdk?.setupLocalVideo(VideoCanvas(surface, RENDER_MODE_HIDDEN, 0))
        } else {
            AgoraRTC.instance()._sdk?.setupRemoteVideo(VideoCanvas(surface, RENDER_MODE_HIDDEN, uid))
        }
    }
